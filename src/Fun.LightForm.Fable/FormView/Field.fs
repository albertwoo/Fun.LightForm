...
let fieldLabel cs label =
    div </> [
        match cs with
        | [] -> Styles [ Color "gray"; FontSize "0.9rem" ]
        | cs -> Classes cs
        Text label
    ]
...
let fieldError cs es =
    [
        for e in es ->
            div </> [
                match cs with
                | [] -> Styles [ Color "red"; FontSize "0.8rem"; Opacity "0.8" ]
                | cs -> Classes cs
                Text e
            ]
    ]
...
let inputField (props: InputProp<_> list) =
    ...
    let inputView = 
        input [
            ...
            | [] -> Styles [ Width "100%"; Margin "2px 0"; Padding "2px 5px"; BackgroundColor "#f1f1f1" ]
            | cs -> classes cs
            Type (
                match value with
    ...
    simpleField [
        ...
        SimpleFieldProp.FieldView (
            div </> [
                ...
                | [] -> Styles [ Display DisplayOptions.Flex ]
                | x  -> yield! x
                Children [
                    match leftView with
                    | Some v -> v
...
let textAreaField (props: TextAreaProp list) =
    ...
    let fieldView = 
        textarea </> [
            ...
            |> function
                | [] -> Styles [ Width "100%"; Margin "2px 0"; Padding "2px 5px"; BackgroundColor "#f1f1f1" ] |> unbox<IHTMLProp>
...
let dropdown =
    FunctionComponent.Of(
        (fun (props: DropdownProp list) ->
            ...
            div </> [
                Styles [ Position PositionOptions.Relative ]
                OnMouseEnter toggel
                OnMouseLeave toggel
                Children [
                    div </> [
